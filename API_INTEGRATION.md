...
1. **Upload Size Limits**: Extremely large single-PUT uploads are split locally and uploaded directly to Telegram using a robust chunked algorithm (`UploadSaveBigFilePart`). This approach safely survives TCP socket closures over proxies with micro-retries. Multipart S3 uploads are currently stubbed.
2. **Download Latency (Proxies)**: If the MinIO server is connected to Telegram via an MTProto Proxy, downloads might experience momentary stutters as the server actively reconstructs dropped chunks using a similar proxy-resilient chunk loop. S3 SDKs with default timeout settings might need extended timeout values.
